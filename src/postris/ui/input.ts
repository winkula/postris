...
export class Input {
    ...
    constructor() {
        ...
        window.addEventListener("touchend", (event: TouchEvent) => {
            ...
            const delta = getVec(event.changedTouches[0]).sub(this.touchstartPosition);
    ...
    private isBottom = (y: number, threshold: number) => y > this.height * (1 - threshold);
    private isLeft = (x: number, threshold: number) => x < this.width * threshold;
    private isRight = (x: number, threshold: number) => x > this.width * (1 - threshold);
}
