...
export class Gfx {
  ...
  constructor(dimensions: Vec) {
    ...
    this.preview.position.add(
      new Vector3(this.dimensions.x + pieceHudPos.x, pieceHudPos.y, pieceHudZ)
    );
    this.hold = this.moveToCenter(new Group());
    this.hold.position.add(
  ...
  async init() {
    ...
    const urls = [dispobox, fragile, box, zalando, letters, box2, eco];
  ...
  renderPreview(preview: Tetrimino) {
    ...
    preview
      .blocks()
      .forEach((block) => this.preview.add(createBlock(block, texture)));
  }
  ...
  renderHold(tetrimino?: Tetrimino) {
    ...
    tetrimino
      .blocks()
      .forEach((block) => this.hold.add(createBlock(block, texture)));
  }
  ...
  renderText(state: State) {
    ...
    const timeStr = `${Math.floor(state.time / 60)
      .toString()
  ...
  animateCamera() {
    ...
    const animation = this.mixer.clipAction(
      createTweenAnimation("camera", 5, before, after)
    );
